...
:: 5. 启动后端 (端口 8001)
echo [*] 启动后端服务 (端口 8001)...
start "SoulHealth-Backend" cmd /k "cd /d %~dp0 && python -m uvicorn pipeline:app --host 0.0.0.0 --port 8001"
ping 127.0.0.1 -n 3 >nul
echo [OK] 后端服务已启动: http://localhost:8001

:: 6. 启动前端 (端口 5173)
echo [*] 启动前端服务 (端口 5173)...
start "SoulHealth-Frontend" cmd /k "cd /d %~dp0soulhealth-frontend-stage1\soulhealth-frontend && npm run dev"
